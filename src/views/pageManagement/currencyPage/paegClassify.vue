...
export default {
  ...
  methods:{
    ...
    getList(roleType,sort){
      let _this = this
      _this.$http.post(_this.baseUrl + _this.url.Common.GetCateList,{
        token:_this.$utils.getToken(),
    ...
    submitClassify(formName){
      ...
      _this.$refs[formName].validate((valid)=>{
        ...
        if(_this.dialogClassifyText=='新增'){
          url = _this.baseUrl + _this.url.Common.AddCate
        }else if(_this.dialogClassifyText=='编辑'){
          url = _this.baseUrl + _this.url.Common.EditCateById
    ...
    changeStatus(status,id){
      ...
      _this.$http.post(_this.baseUrl + _this.url.Common.ChangeCateStatusByIds,{
        token:_this.$utils.getToken(),
        ids:id,
        status:status
      }).then((res)=>{
    ...
    editManage(id){
      let _this = this
      _this.classifyId = id
      _this.$http.post(_this.baseUrl + _this.url.Common.GetCateInfoById,{
        id:id,
        token:_this.$utils.getToken()
      }).then((res)=>{
        if(res.data.ret==200){
    ...
    delManage(id){
      let _this = this
       _this.$confirm('此操作将永久删除该信息, 是否继续?','提示',{
         ...
         _this.$http.post(_this.baseUrl + _this.url.Common.DelCateByIds,{
           token:_this.$utils.getToken(),
            ids:id
         }).then((res)=>{
           if(res.data.ret==200){
