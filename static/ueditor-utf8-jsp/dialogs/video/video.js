	...
	UploadFile.prototype = {
		...
		initUploader: function() {
				...
				switch(type) {
					...
					case 'startUpload':
						...
						var params = utils.serializeParam(editor.queryCommandValue('serverparam')) || '',
							url = utils.formatUrl(actionUrl + (actionUrl.indexOf('?') == -1 ? '?' : '&') + 'encode=utf-8&' + params + '&type=common&name=' + inputFile.name + localStorage['userId' + sessionStorage['roleTypeId']]);
			...
			uploader.on('uploadBeforeSend', function(file, data, header) {
				...
				header['timestamp'] = Math.round(new Date().getTime() / 1000);
        header['asd'] = 12;
				// header['token'] = localStorage['token' + sessionStorage['roleTypeId']];
        header['sign'] = md5('/datacenter/public/v1/fileupload/image' + Math.round(new Date().getTime() / 1000));
				// header['sign'] = md5('/datacenter/public/v1/fileupload/image' + Math.round(new Date().getTime() / 1000) + localStorage['userId' + sessionStorage['roleTypeId']] + sessionStorage['roleType'] + localStorage['randomAccessCode' + sessionStorage['roleTypeId']]);
			});
				...
				try {
					...
					if(json.status == '00000000') {
						uploadVideoList.push({
							...
							'type': json.data.name,
							'original': json.data.fileUrl,
							'videoId': json.data.fileId
						});
